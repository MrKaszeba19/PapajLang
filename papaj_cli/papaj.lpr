...
begin
    Writeln();
    Writeln(StdErr, 'papaj has been halted by SIGINT.');
    Halt(1);
end;
{$ENDIF}
...
begin
    writeln('Papaj - PapajScript Interpreter.'); 
    if (RPN_update <= 0) 
    ...
    writeln('Since 2017/11/24. Written in FreePascal. :)');
...
begin
    ...
    if (not RPN_isStable) 
        then writeln('unstable ('+RPN_codename+'), '+RPN_date+' update, for '+RPN_targetOS+' '+RPN_targetCPU+'.')
        else if (RPN_update <= 0) 
            then writeln(''+RPN_version+' ('+RPN_codename+'), for '+RPN_targetOS+' '+RPN_targetCPU+'.')
            else writeln(''+RPN_version+' ('+RPN_codename+'), update #'+IntToStr(RPN_update)+', for '+RPN_targetOS+' '+RPN_targetCPU+'.');
end;
...
procedure show_help();
begin
    writeln('SYNTAX: papaj do "quoted_PS_code"');
    writeln('        papaj FILENAME');
    writeln('');
    writeln('Run ''papaj help'' to display this again.');
    writeln('Run ''papaj expression'' to obtain info about making RPN expressions.');
    writeln('Run ''papaj functions'' to obtain info about available functions.');
    writeln('Run ''papaj functions PAGE'' to obtain info about functions (specific page).');
    writeln('Run ''papaj packages'' to check the packages for PapajScript');
    writeln('Run ''papaj run FILENAME'' to run a PS script file');
...
begin
	...
	writeln('Type ''papaj packages [page_num]'' to obtain info about specific packages, e.g. ''rpn packages 1''');
    writeln('Type ''papaj packages all'' to obtain info about all packages.');
end;
