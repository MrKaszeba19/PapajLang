...
begin
    ...
    begin
        ...
        case db.Commands[at][i].Name of
            _EVAL : begin
                //write(#9, db.Commands[at][i].StrParam);
...
var i, at : LongInt;
begin
    writeln('length ', Length(cmds.Commands));
    if (Length(cmds.Commands) > 0) then 
    begin
        for at := 0 to Length(cmds.Commands)-1 do 
        begin
            writeln('set ', at);
            for i := 0 to Length(cmds.Commands[at])-1 do 
            begin
                write(cmds.Commands[at][i].Name, #9);
                if (cmds.Commands[at][i].Name2 <> _NONE) then write(cmds.Commands[at][i].Name2, #9);
                writeln();
            end;
            writeln();
        end;
...
begin
    ...
    {build} buildCommands(input, Scripts[index]);
