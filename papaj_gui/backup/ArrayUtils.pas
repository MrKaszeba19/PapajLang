...
begin
    if factor = 0 then begin
        ...
        s := 0.0;
        for i := 0 to Length(tab)-1 do
  	        s := s + ComplexNumbers.pow(tab[i].Num, factor);
        table_avg_power := ComplexNumbers.pow(s/Length(tab), 1/factor);
    end;
end;
...
begin
    ...
    m := table_avg(tab);
    for i := 0 to Length(tab)-1 do
        s := s + Complexnumbers.pow(tab[i].Num - m, k);
    Result := s/Length(tab);
end;
...
begin
    ...
    if s = 0
        then Result := 0
        else Result := (table_moment(tab, 3))/(ComplexNumbers.pow(s,3));
end;
...
begin
    ...
    if s = 0
        then Result := 0
        else Result := (table_moment(tab, 4))/(pow(ComplexNumbers.s,4));
end;
