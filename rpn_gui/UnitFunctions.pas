	...
	case i of
        ...
        'callWhile' : begin
            ...
            begin
                ...
                StrEbx := stack_pop(pocz[sets.StackPointer]).Str;
                doWhile(StrEbx, EntEax.Str, pocz, sets, vardb);
            end else begin
                if (sets.StrictType) and (assertEntityLocated(pocz[sets.StackPointer], stack_get(pocz[sets.StackPointer]), TEXP, i)) then Exit;  
                StrEbx := stack_pop(pocz[sets.StackPointer]).Str;
                if (sets.StrictType) and (assertEntityLocated(pocz[sets.StackPointer], stack_get(pocz[sets.StackPointer]), TFUN, i)) then Exit;  
                EntEax := stack_pop(pocz[sets.StackPointer]);
                doDoWhile(StrEbx, EntEax.Str, pocz, sets, vardb);
            end;
            //while (True) do
            //begin
        ...
        'callUntil' : begin
            ...
            begin
                ...
                if (sets.StrictType) and (assertEntityLocated(pocz[sets.StackPointer], stack_get(pocz[sets.StackPointer]), TEXP, i)) then Exit;  
                StrEbx := stack_pop(pocz[sets.StackPointer]).Str;
                doUntil(StrEbx, EntEax.Str, pocz, sets, vardb);
            end else begin
                if (sets.StrictType) and (assertEntityLocated(pocz[sets.StackPointer], stack_get(pocz[sets.StackPointer]), TEXP, i)) then Exit;  
                StrEbx := stack_pop(pocz[sets.StackPointer]).Str;
                if (sets.StrictType) and (assertEntityLocated(pocz[sets.StackPointer], stack_get(pocz[sets.StackPointer]), TFUN, i)) then Exit;  
                EntEax := stack_pop(pocz[sets.StackPointer]);
                doDoUntil(StrEbx, EntEax.Str, pocz, sets, vardb);
            end;
        end;        
...
begin
	...
	case i of
        ...
        'Math.modularPower' : begin
            ...
            if not (sets.Autoclear) then stack_push(pocz[sets.StackPointer], buildNumber(y));
            if not (sets.Autoclear) then stack_push(pocz[sets.StackPointer], buildNumber(z));
            stack_push(pocz[sets.StackPointer], buildNumber(modularPower(trunc(x), trunc(y), trunc(z))));
        end;
        'Math.modPower' : begin
            if (sets.StrictType) and (assertNaturalLocated(pocz[sets.StackPointer], stack_get(pocz[sets.StackPointer]), i)) then Exit;
            z := stack_pop(pocz[sets.StackPointer]).Num;
        ...
        'Math.modularInverse' : begin
            ...
            if not (sets.Autoclear) then stack_push(pocz[sets.StackPointer], buildNumber(x));
            if not (sets.Autoclear) then stack_push(pocz[sets.StackPointer], buildNumber(y));
            stack_push(pocz[sets.StackPointer], buildNumber(modularInverse(trunc(x), trunc(y))));
        end;
        ...
        'Math.coprimes' : begin
            ...
            if not (sets.Autoclear) then stack_push(pocz[sets.StackPointer], buildNumber(x));
            if not (sets.Autoclear) then stack_push(pocz[sets.StackPointer], buildNumber(y));
            stack_push(pocz[sets.StackPointer], buildBoolean(gcd(x, y) = 1));
        end;
        'Math.isCarmichael' : begin
            if (sets.StrictType) and (assertNaturalLocated(pocz[sets.StackPointer], stack_get(pocz[sets.StackPointer]), i)) then Exit; 
            y := stack_pop(pocz[sets.StackPointer]).Num;
            if not (sets.Autoclear) then stack_push(pocz[sets.StackPointer], buildNumber(y));
            stack_push(pocz[sets.StackPointer], buildBoolean(num_isCarmichael(trunc(y))));
        end;
		else begin
            Found := false;
        end;
    ...
    case LeftStr(i, 1) of
    	'$' : begin
      		if (RightStr(i, Length(i)-1) <> '') then begin
                ...
                stack_push(pocz[sets.StackPointer], raiseGetUnnamedVariable());
      		end;
     	end;
     	'>' : begin 
     		if (RightStr(i, Length(i)-1) <> '') then begin
                ...
                begin
                    ...
                    stack_push(pocz[sets.StackPointer], raiseSetInvalidVariable(i, StrEax));
                end;
      		end else begin
        		stack_push(pocz[sets.StackPointer], raiseSetUnnamedVariable());
      		end;
     	end;
     	'?' : begin 
     		if (RightStr(i, Length(i)-1) <> '') then begin
